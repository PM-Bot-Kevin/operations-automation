...
- 默认决策顺序固定是：`云端正式站 -> 本机正式站 -> 本机权限例外站`
- 能做成云端正式站的，默认不要继续依赖这台电脑常开
- 只有确实依赖本机文件、Eagle、桌面软件或本机运行环境时，才允许做成本机正式站
- 只有明确命中 macOS 权限限制、必须从用户会话拉起才能稳定工作时，才允许归为本机权限例外站
- 新网站上线时，必须把它属于哪一类站点写进维护文档；如果是本机正式站或本机权限例外站，必须同时补自动恢复方案
- 如果现有 3 类都不合适，必须先得到用户明确确认，才允许新增第 4 类长期方案
...
- 这条“任务前后窗口差异 + 只关闭本轮新开的目标页窗口 + 快照缺失不盲关”的收尾规则，已经下沉成千帆后台共用能力；后续新增千帆任务默认优先复用，不再各自维护一套关窗逻辑
