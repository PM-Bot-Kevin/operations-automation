...
- 站点分型默认固定先按 3 类长期方案评估：`云端正式站 -> 本机正式站 -> 本机权限例外站`。
- 先判断能不能做成云端正式站；只有确实依赖这台 Mac 的本机文件、Eagle、桌面软件或本机运行环境时，才允许做成本机正式站。
- 只有明确命中 macOS 权限限制、必须从用户会话拉起才能稳定工作的站，才允许归为本机权限例外站。
- 新网站上线时，必须在维护文档里明确写出它属于哪一类站点；如果是本机正式站或本机权限例外站，必须同时补自动恢复方案，不能留到以后再补。
- 如果现有 3 类都不合适，必须先明确说明为什么不合适、为什么必须新增方案，以及对稳定性、维护成本、本机负担的影响；只有得到用户明确确认后，才允许开发、落文档、上线和后续维护。
